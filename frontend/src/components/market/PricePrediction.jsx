        ...
        <form onSubmit={handlePredict} className="flex flex-col gap-5">
          <div>
            ...
            <select
              ...
              required
              className="w-full px-4 py-3 border border-white/10 rounded-2xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-400 bg-white/10"
            >
          ...
          <div>
            <label className="text-sm font-semibold text-white/70 mb-1.5 block">🗓️ Season</label>
            <select
          ...
          <div>
            <label className="text-sm font-semibold text-white/70 mb-1.5 block">📅 Year</label>
            <input
              type="number"
        ...
        {predLoading && (
          <div className="bg-black/40 backdrop-blur-md rounded-3xl p-8 border border-white/10 flex items-center justify-center min-h-[300px]">
            <div className="text-center space-y-3">
              ...
              <p className="text-white/40 text-sm animate-pulse">Running prediction model...</p>
            </div>
          </div>
        )}

        {prediction && !predLoading && (
          <div className="bg-black/40 backdrop-blur-md rounded-3xl p-8 shadow-lg border border-white/10 space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              {[prediction.crop, prediction.state, prediction.season, prediction.year].map((v, i) => (
                <span
                  key={i}
                  className="text-xs bg-emerald-500/20 text-emerald-300 px-3 py-1 rounded-full font-medium border border-emerald-500/30"
                >
                  {v}
                </span>
              ))}
            ...
            <div className="text-center bg-gradient-to-br from-emerald-500/20 to-green-500/20 rounded-2xl py-6 px-4 border border-emerald-500/30">
              <p className="text-white/50 text-xs mb-2 uppercase tracking-wide">Predicted Price</p>
              <p className="text-5xl font-extrabold text-emerald-600">
                ₹{prediction.predicted_price?.toLocaleString() ?? "—"}
              </p>
              <p className="text-white/40 text-xs mt-2">per quintal</p>
            </div>
            ...
            {prediction.confidence != null && (
              <div className="bg-white/5 rounded-2xl p-4">
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-white/60 font-medium">Model Confidence</span>
            ...
            {prediction.min_price != null && prediction.max_price != null && (
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-red-500/20 rounded-2xl p-4 text-center border border-red-500/30">
                  <p className="text-xs text-white/40 mb-1">Low Estimate</p>
                ...
                <div className="bg-green-500/20 rounded-2xl p-4 text-center border border-green-500/30">
                  <p className="text-xs text-white/40 mb-1">High Estimate</p>
            ...
            {prediction.advice && (
              <div className="bg-amber-500/20 border border-amber-500/30 rounded-2xl p-4 flex gap-3">
        ...
        {!prediction && !predLoading && (
          <div className="bg-black/40 backdrop-blur-md rounded-3xl p-8 border border-white/10 flex items-center justify-center min-h-[300px]">
            <div className="text-center space-y-2">
              ...
              <p className="text-white/40 text-xs">Fill the form to get an AI price forecast</p>
            </div>
          </div>
        )}
      </div>
