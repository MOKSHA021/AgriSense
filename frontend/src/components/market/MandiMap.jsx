...
// ── Sub: Routing ──
function RoutingMachine({ farmerPos, mandiPos, onRouteFound }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    const control = L.Routing.control({
      waypoints: [L.latLng(...farmerPos), L.latLng(...mandiPos)],
      routeWhileDragging: false,
      addWaypoints: false,
      draggableWaypoints: false,
      fitSelectedRoutes: true,
      showAlternatives: false,
      lineOptions: { styles: [{ color: "#f59e0b", weight: 5, opacity: 0.85 }] },
      createMarker: () => null,
    });
    ...
    control.on("routesfound", (e) => {
      const r = e.routes[0];
      onRouteFound({
        distanceKm: (r.summary.totalDistance / 1000).toFixed(1),
        durationMin: Math.round(r.summary.totalTime / 60),
      });
    });
...
const MandiMap = ({
  farmerLocation,
  farmerAddress,
  mandiLocation,       // red pin — set by clicking a card or searching
  selectedMandi,
  clickMode,
  onMapClick,
  flyTarget,
  ...
  return (
    ...
    >
      ...
      {/* 🔴 Mandi Marker — shown when a card is clicked */}
      {mandiLocation && (
