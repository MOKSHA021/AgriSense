  ...
  const autoDetectEverything = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords }) => {
        ...
        try {
          /* Weather and Location from Backend API */
          try {
            const { data } = await API.get('/weather/farm-forecast', { params: { lat, lon } });
            
            temp = Math.round(data.current?.temperature ?? 25);
            humidity = Math.round(data.current?.humidity ?? 60);
            monthlyRain = data.rainfall30Days || 80;
            
            weatherSuccess = true;
            
            // Backend also natively reverse-geocodes the location
            detectedDistrict = data.location?.district || data.location?.displayName || "Unknown";
            detectedDistrict = detectedDistrict.replace(/\bDistrict\b/gi, '').trim();
          } catch (weatherErr) {
            console.error('Backend Weather API failed:', weatherErr);
          }
          ...
          // Geocoding is now handled in the weather API call above
  ...
  const analyseSoilImage = async (file) => {
    ...
    try {
      ...
      try {
        ...
        try {
          const { data } = await API.get('/weather/farm-forecast', { params: { lat, lon } });
          const temp = Math.round(data.current?.temperature ?? 25);
          const humidity = Math.round(data.current?.humidity ?? 60);
          const rain = data.rainfall30Days || 150;
          
          setParams(p => ({
            ...p,
            temperature: temp,
            humidity: humidity,
            rainfall: Math.min(300, rain),
          }));
        } catch (backendWeatherErr) {
          console.error('Backend Weather API failed during soil analysis:', backendWeatherErr);
        }
      } catch (weatherErr) {
